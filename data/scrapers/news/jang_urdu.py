"""Jang Urdu news scraper for child trafficking and abuse articles.

Scrapes Daily Jang (jang.com.pk), Pakistan's largest Urdu-language
newspaper. Fetches the latest news listing page, extracts article
links, fetches each article's full text, and filters by Urdu
trafficking keywords.

Source: https://jang.com.pk/latest-news
...
class JangUrduScraper(BaseScraper):
    ...
    async def _fetch_article_links(self) -> list[str]:
        ...
        try:
            ...
            links: list[str] = []
            # Jang uses heading links and list-item links for articles
            for selector in ["h2 a", "h3 a", ".news-card a", ".story-link", "a.open-section"]:
                for anchor in soup.select(selector):
                    href = anchor.get("href", "")
                    if not href or href == "#":
                        continue
                    if href.startswith("/"):
                        href = f"https://jang.com.pk{href}"
                    if "jang.com.pk" in href and href not in links:
                        links.append(href)
            ...
            if not links:
                for anchor in soup.find_all("a", href=True):
                    href = anchor["href"]
                    if href.startswith("/") and "/news-" in href:
                        full_url = f"https://jang.com.pk{href}"
    ...
    async def _fetch_article(self, url: str) -> dict[str, Any]:
        """Fetch and parse a single Jang article.
        ...
        try:
            ...
            # Extract title
            title = ""
            title_tag = soup.find("h1") or soup.find("h2", class_="detail-heading")
            if title_tag:
            ...
            if time_tag:
                ...
                date_span = soup.find("span", class_="date") or soup.find("span", class_="time")
                if date_span:
    ...
    async def scrape(self) -> list[dict[str, Any]]:
        """Execute the Jang Urdu scraping pipeline.
    ...
    def validate(self, record: dict[str, Any]) -> bool:
        """Validate a scraped Jang article record.
